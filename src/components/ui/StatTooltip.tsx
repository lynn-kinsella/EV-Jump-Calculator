...
function TooltipContainer({ columnData, statName, mouseCoords, target }: TooltipContainerProps) {
    ...
    if (columnData) {
        return (
            <div className="text-base flex flex-row items-top justify-center w-fit absolute z-20 pointer-events-none"
                style={getOffset(mouseCoords)}>
                <TooltipPointer direction={getDirection(mouseCoords[1])} ></TooltipPointer>
                <TooltipContents columnData={columnData} statName={statName} direction={getDirection(mouseCoords[1])} target={target}></TooltipContents>
            </div >
        );
    }
}
...
function TooltipContents({ columnData, target, statName, direction }: TooltipContentsProps) {
    ...
    return (
        <div className="bg-blue-100 w-52 h-fit border-blue-400 border-4" style={getBorderStyle(direction)}>
            <div className="px-4 flex flex-col gap-2">
                <div className="flex-col flex content-center">
                    {/* % Range */}
                    <span className="text-center text-lg">{Math.round(1000 * columnData.calcs.low / target.calcData.stats.hp) / 10}% - {Math.round(1000 * columnData.calcs.high / target.calcData.stats.hp) / 10}%</span>
